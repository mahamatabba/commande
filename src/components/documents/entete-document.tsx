 ...
 * bons de commande.
 *
 * Les variantes `print:` resserrent le rythme vertical sur papier. À l'écran
 * l'espace est gratuit ; sur A4 il décide si le document tient sur une seule
 * feuille, et l'en-tête est le bloc le plus haut du document.
 */
export function DocumentHeader({
  ...
  return (
    <header className="mb-5 print:mb-3 print:break-inside-avoid">
      <div className="feuille-a4__bande grid grid-cols-[auto_1fr_auto] items-center gap-6 py-5 print:py-3">
      ...
      <div className="feuille-a4__bande flex items-end justify-between border-t border-[#E6E3DD] py-4 print:py-3">
...
/**
 * Bloc de signatures commun aux quatre documents imprimables. Le côté droit
 * est toujours AEI ; seul le cosignataire change, d'où les deux libellés en
 * paramètres.
 *
 * L'espace laissé au-dessus du filet doit rester suffisant pour une signature
 * manuscrite : il est donc large à l'écran, et resserré à l'impression, où le
 * même blanc peut coûter une deuxième feuille.
 */
export function BlocSignatures({
  ...
  return (
    <section className="grid grid-cols-2 gap-8 text-xs print:break-inside-avoid">
      <div>
        <p className="mb-10 font-semibold text-[#4A4844] print:mb-7">{titre}</p>
        <div className="border-t border-[#D9D6D0] pt-1 text-[#9C9A95]">{mention}</div>
      </div>
      <div>
        <p className="mb-10 font-semibold text-[#4A4844] print:mb-7">Pour {AEI_INFO.nom}</p>
...
export function DocumentFooter() {
  return (
    <footer className="mt-6 border-t-[3px] border-[#00AEEF] print:mt-4 print:break-inside-avoid">
      <div className="feuille-a4__bande flex flex-wrap items-stretch justify-between gap-4 py-3 print:py-2">
        ...
        {/* Le badge déborde jusqu'au bord du papier : la marge négative est
            calculée sur --feuille-marge et non écrite en dur, sinon elle
            cesserait de coïncider avec la marge dès qu'on la change — ce qui
            est justement le cas à l'impression. */}
        <div className="-my-3 flex shrink-0 items-center bg-[#FAA755] px-5 text-xs font-bold text-white mr-[calc(var(--feuille-marge)*-1)] print:-my-2">
